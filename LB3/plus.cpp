...
vector<vector<double>> GaussianKernel(int size, double sigma) { //создает двумерное ядро Гаусса, используя формулу Гаусса. Мы нормализуем ядро, чтобы сумма всех весов была равна 1
    ...
    for (int x = -halfSize; x <= halfSize; ++x) {
        for (int y = -halfSize; y <= halfSize; ++y) {
            kernel[x + halfSize][y + halfSize] = (1 / (2  * sigma * sigma)) * exp(-(x * x + y * y) / (2 * sigma * sigma));
